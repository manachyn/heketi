...
func TestReplaceBrickInReplicatedVolumeError(t *testing.T) {
	tmpfile := "/home/imanachyn/go/src/github.com/heketi/heketi/my/heketi.db"
	app := NewTestApp(tmpfile)
	defer app.Close()
	...
	var be *BrickEntry
	err = app.db.View(func(tx *bolt.Tx) error {
		v, _ = NewVolumeEntryFromId(tx, "752456292308433757567255021f3bc2")
		for _, brick := range v.Bricks {
			be, err = NewBrickEntryFromId(tx, brick)
			if err != nil {
				return err
